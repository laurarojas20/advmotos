...
const NavBar = () => {
  ...
  <Navbar.Collapse id="responsive-navbar-nav">
    <Nav className="me-auto">
      <Nav.Link><Link to={`/indumentaria/indumentaria`}> Indumentaria </Link></Nav.Link>
      <Nav.Link><Link to={`/performance/performance`}> Performance </Link></Nav.Link>
    ...
    <Nav.Link> <Link to={`/carrito`} >
      ...
      <span> { 
        carritoProductos.length === 0 ? 0 : carritoProductos.length
      } </span>
      </button> 
      </Link>
    </Nav.Link>
